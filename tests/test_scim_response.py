...
# - because .extension and .examples are populated dynamically
...
def test_validating_invalid_example_user():
    ...
    pattern_invalid_str = re.compile(
        r"\'Single-value attribute:userName\' is required at the following "
        r'location \'\["urn:ietf:params:scim:schemas:core:2.0:User", '
        r'"userName"\]\' but found \'{"'
    )
    assert re.search(pattern_invalid_str, str(assert_error)) is not None
...
def test_validating_valid_example_account():
    ...
    assert assert_error is not None
    pattern_invalid_str = re.compile(
        r"\'Single-value attribute:accountPackage\' is required at the "
        r"following location \'\["
        r'"urn:huddle:params:scim:schemas:extension:2.0:Account", "package", '
        r'"accountPackage"\]\' but found \'{}\''
    )
    assert re.search(pattern_invalid_str, str(assert_error)) is not None
...
def test_get_invalid_meta_schema():
    ...
    # given a response with a validate schemas attribute - both user and
    # group (for the sack of testing)
...
def test_get_meta_schema_without_definitions(schema_without_definition):
    # given a response without a validate schemas attribute is supplied from
    # the row test
